...
export default class Carousel extends Component {
  ...
  _navigatePage = () => {
    if (this.props.onNavigatePage) {
      this.props.onNavigatePage();
    }
  }

  _renderArrows = () => {
    let { currentPage } = this.state;
    let { onNavigatePage } = this.props;
    const { childrenLength } = this.state;
    if (currentPage < 1) {
        ...
        <View style={[styles.arrowsContainer, this.props.arrowsContainerStyle]} pointerEvents="box-none">
          ...
          >
            <Text style={this.props.rightArrowStyle}>
              {this.props.rightArrowText ? this.props.rightArrowText : currentPage === childrenLength - 1 ? 'Finish' : 'Next'}
            </Text>
          </TouchableOpacity>
        </View>
      </View>
